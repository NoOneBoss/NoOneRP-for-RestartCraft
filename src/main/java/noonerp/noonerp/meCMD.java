...
public class meCMD implements CommandExecutor {
    ...
    public meCMD(NoOneRP plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(!sender.hasPermission("noonerp.me")) {
            sender.sendMessage(plugin.getConfig().getString("messages.you_dont_have_permission").replace("&","§"));
            return true;
        }
        if (args.length == 0) {
            sender.sendMessage(plugin.getConfig().getString("messages.bad_usage_me").replace("&","§"));
            return true;
        }
        String text = "";
        ...
        int radius = plugin.getConfig().getInt("radius.me");
        ...
        sender.sendMessage(plugin.getConfig().getString("messages.commands.me").replace("&","§").replace("%player%", sender.getName()).replace("%text%", text));
        return true;
    }
}
